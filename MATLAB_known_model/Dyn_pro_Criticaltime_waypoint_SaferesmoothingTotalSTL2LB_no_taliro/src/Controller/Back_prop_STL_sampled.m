function [gradient_STL, sampled_T] = Back_prop_STL_sampled(s0, C, D, param, dim, T, Map, N_sample)
...
Trajectory  = [s0 , [s{:}]];
[Info , rrr ] = Your_robustness(Trajectory, Map);

what_predicate = Info(1);
critical_time = Info(2);


sss = Trajectory(:,critical_time+1);
rrrr = Map.C(what_predicate, :)*sss+Map.d(what_predicate, 1);
if rrrr ~= rrr
    error('wrong critical time');
end


grad_state  = Map.C(what_predicate, :);
...
if critical_time == 0
    ...
    for i=2:N_STL
        ...
        for j=num_layer+1:-1:2
            der  = der * Weights{j}*diag(sparse(1- Postactivate{j-1,sampled_STL(i)}.^2));
        end
    ...
    for param_index=1:num_layer
        ...
        for m=1:M
            ...
            for i=2:N_STL
                ...
                der =1;
                for j=num_layer+1:-1:param_index+2
                    der  = der * Weights{j}*diag(sparse(1-Postactivate{j-1,sampled_STL(i-1)+1}.^2));
                end
                der = der * Weights{param_index+1};
