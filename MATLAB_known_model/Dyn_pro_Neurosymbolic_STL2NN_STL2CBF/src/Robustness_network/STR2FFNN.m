function [W, L , B]  =  STR2FFNN(str, Map , T)
...
if isempty(ands) && ~isempty(ors)   %%% include the remainder
    ...
    [W_add, L_add] = pure_poslin('max' , number_or); 
    add_L = length(L_add);
    [Ws{1}, Ls{1}, Bs{1}] = STR2FFNN(unpack(ors{1}), Map , T);
    H(1) = length(Ls{1});
    for i=2:number_or
        [Ws{i}, Ls{i}, Bs{i}] = STR2FFNN(unpack(ors{i}), Map , T);
        H(i) = length(Ls{i});
    end
    Height = max(H);
    for i=1:number_or
    ...
    if Height==0   %%%% In case leaf is added with non-leaf siblings then it certainly obtained its purelin and thus this process is not necessary anymore!!!
        B = W_add{1}*B;
    end
...
elseif isempty(ors) && ~isempty(ands)   %%% and we generate the tree like this if all the events are contributing in conjunction
    ...
    [W_add, L_add] = pure_poslin('min' , number_and); 
    add_L = length(L_add);
    [Ws{1}, Ls{1} , Bs{1}] = STR2FFNN(unpack(ands{1}), Map , T);
    H(1) = length(Ls{1});
    for i=2:number_and
        [Ws{i}, Ls{i}, Bs{i}] = STR2FFNN(unpack(ands{i}), Map , T);
        H(i) = length(Ls{i});
    end
    Height = max(H);
    for i=1:number_and
