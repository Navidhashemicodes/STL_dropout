...
for iter = 1:Num
    ...
    if mod(iter,50)==0
        ...
        for i=1:NMN
            s0 = -ones(2,1)+0.0005*randn(2,1);
            [~, Hard(i)] = STL_obj(s0, param{iter}, dim, T, STL2CBF, b , STL2NN, smooth , hard);
        end
        success = 0.01*(sum(Hard>0));
        disp(['Hard robustness success rate at iteration ' num2str(iter) ' is ' num2str(success) '.' ]);
        if success>0.95
            break;
        end
    end
...
save('STL2CBF_b30_Test_STL_wiser_STLCG_noisy_badinit_strong_controller2.mat')
% save('STL2CBF_b30_Test_STL_wiser_STLCG_noisy_badinit_limitted_controller.mat')